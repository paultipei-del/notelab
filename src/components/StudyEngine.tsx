...
export default function StudyEngine({ deck, userId }: StudyEngineProps) {
  ...
  if (isComplete && viewMode === 'study') {
    ...
    const msg = stats.correct === stats.total ? 'Perfect session!' : stats.correct > stats.total * 0.8 ? 'Great work!' : 'Keep practicing!'
    ...
    return (
      <div style={{ minHeight: '100vh', background: '#F5F2EC', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', padding: '20px 32px 0' }}>
          ...
          <button style={{ padding: '6px 14px', borderRadius: '20px', border: '1px solid #1A1A18', background: '#1A1A18', color: 'white', fontFamily: 'var(--font-jost), sans-serif', fontSize: '12px', fontWeight: 300, cursor: 'pointer' }}>
            Browse
          </button>
        </div>
            ...
            {deck.cards.map((card, i) => (
              <div key={card.id}
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: '12px' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    {card.type === 'symbol' ? (
                      <p style={{ fontFamily: 'Bravura, serif', fontSize: '36px', color: '#1A1A18', lineHeight: 1.4, marginBottom: browseExpanded === card.id ? '10px' : '0' }}>
                        {card.front}
                        {card.symbolName && <span style={{ fontFamily: 'var(--font-jost), sans-serif', fontSize: '14px', fontWeight: 300, color: '#888780', marginLeft: '12px' }}>{card.symbolName}</span>}
                      </p>
                    ) : (
                  ...
                  <span style={{ fontSize: '12px', color: '#D3D1C7', paddingTop: '2px' }}>{browseExpanded === card.id ? '▲' : '▼'}</span>
                </div>
              </div>
            ))}
          </div>
  ...
  function renderFlipCard() {
    ...
    return <FlipCard card={flipCard as any} revealed={flipRevealed} onReveal={() => setFlipRevealed(true)} />
  }
      ...
      {isFlipMode && (
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '16px', padding: '8px 32px 16px' }}>
          <button onClick={goPrev} disabled={flipIndex === 0}
            style={{ background: 'white', border: '1px solid #D3D1C7', borderRadius: '8px', padding: '10px 24px', fontFamily: 'var(--font-jost), sans-serif', fontSize: '13px', fontWeight: 300, color: flipIndex === 0 ? '#D3D1C7' : '#888780', cursor: flipIndex === 0 ? 'default' : 'pointer' }}>
            ← Prev
          </button>
          ...
          <button onClick={goNext} disabled={flipIndex === flipCards.length - 1}
            style={{ background: 'white', border: '1px solid #D3D1C7', borderRadius: '8px', padding: '10px 24px', fontFamily: 'var(--font-jost), sans-serif', fontSize: '13px', fontWeight: 300, color: flipIndex === flipCards.length - 1 ? '#D3D1C7' : '#888780', cursor: flipIndex === flipCards.length - 1 ? 'default' : 'pointer' }}>
            Next →
          </button>
        </div>
      )}
      ...
      {isFlipMode && flipRevealed && (
        <div style={{ display: 'flex', justifyContent: 'center', gap: '12px', padding: '8px 32px 24px', flexWrap: 'wrap' }}>
          {[
            ...
            <button key={rating}
              onClick={() => { rate(rating); setFlipRevealed(false); setFlipIndex(i => Math.min(i + 1, flipCards.length - 1)) }}
