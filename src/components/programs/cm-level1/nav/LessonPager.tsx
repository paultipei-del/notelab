...
import type { CMLevel1Lesson } from '@/lib/programs/cm-level1/lessons'
...
function lessonBadge(lesson: CMLevel1Lesson): string {
...
export function LessonPager({ prev, next }: LessonPagerProps) {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginTop: 36 }}>
      {prev ? (
        <Link href={`/programs/cm/l1/${prev.slug}`} style={{ textDecoration: 'none' }}>
...
function PagerCard({
  ...
  return (
    ...
    >
      ...
      <span style={{ fontFamily: SERIF, fontSize: 17, fontWeight: 400, color: '#2A2318' }}>{title}</span>
    </div>
  )
}
