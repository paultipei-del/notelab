...
export default function MultipleChoice({ card, options, onAnswer, onReveal }: MultipleChoiceProps) {
  ...
  function getButtonStyle(option: string) {
    ...
    return { ...base, background: 'white', borderColor: '#D3D1C7', color: '#D3D1C7' }
  }
  ...
  const questionContent = () => {
    if (card.type === 'staff' && card.note && card.clef) {
      return (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '12px' }}>
          ...
          <p style={{ fontSize: '13px', fontWeight: 300, color: '#888780', letterSpacing: '0.05em' }}>Identify this note</p>
        </div>
      )
    }
    if (card.type === 'symbol') {
      return (
        <div style={{ textAlign: 'center' }}>
          ...
          {card.symbolLabel && (
            <p style={{ fontSize: '12px', fontWeight: 300, color: '#888780', letterSpacing: '0.05em' }}>
              {card.symbolLabel}
            </p>
          )}
        </div>
    ...
    return (
      <p style={{ fontFamily: 'var(--font-cormorant), serif', fontWeight: 300, fontSize: 'clamp(24px, 4vw, 40px)', textAlign: 'center', letterSpacing: '0.01em', color: '#1A1A18', lineHeight: 1.3 }}>
        {card.front}
      </p>
    )
  }
