...
export default function AuthModal({ onClose, onSuccess }: AuthModalProps) {
  ...
  if (confirmed) {
    return (
      <div style={{ position: 'fixed', inset: 0, background: 'rgba(26,26,24,0.4)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
        ...
        <div style={{ background: '#F5F2EC', borderRadius: '16px', padding: '48px 40px', maxWidth: '420px', width: '90%', textAlign: 'center' }}>
          <div style={{ fontSize: '36px', marginBottom: '16px' }}>✉️</div>
          <h2 style={{ fontFamily: 'var(--font-cormorant), serif', fontWeight: 300, fontSize: '28px', marginBottom: '12px', color: '#1A1A18' }}>
            Check your email
          </h2>
          <p style={{ fontSize: '14px', fontWeight: 300, color: '#888780', lineHeight: 1.7, marginBottom: '28px' }}>
  ...
  return (
    <div
      ...
      onClick={e => e.target === e.currentTarget && onClose()}
    >
      <div style={{ background: '#F5F2EC', borderRadius: '16px', padding: '40px', maxWidth: '420px', width: '90%', boxShadow: '0 8px 48px rgba(26,26,24,0.2)' }}>
        ...
        <h2 style={{ fontFamily: 'var(--font-cormorant), serif', fontWeight: 300, fontSize: '28px', color: '#1A1A18', marginBottom: '24px' }}>
          {mode === 'signin' ? 'Welcome back' : 'Join NoteLab'}
        </h2>

        <div style={{ marginBottom: '14px' }}>
          <label style={{ fontSize: '11px', fontWeight: 400, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#888780', display: 'block', marginBottom: '6px' }}>
            Email
          </label>
          <input
            style={inputStyle}
        ...
        <p style={{ textAlign: 'center', fontSize: '12px', fontWeight: 300, color: '#888780', marginTop: '20px' }}>
          {mode === 'signin' ? "Don't have an account? " : 'Already have an account? '}
          <button
            onClick={() => { setMode(mode === 'signin' ? 'signup' : 'signin'); setError('') }}
            style={{ background: 'none', border: 'none', color: '#1A1A18', cursor: 'pointer', fontSize: '12px', fontWeight: 400, textDecoration: 'underline' }}
          >
