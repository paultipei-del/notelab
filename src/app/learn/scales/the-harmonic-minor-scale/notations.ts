function labeledNote(step: string, octave: number, label: string, alter = 0, acc: string | null = null): string {
...
function plainNote(step: string, octave: number, alter = 0, acc: string | null = null): string {
  const alterTag = alter !== 0 ? `<alter>${alter}</alter>` : ''
  const accTag = acc ? `<accidental>${acc}</accidental>` : ''
...
export const A_HARMONIC_MINOR_XML = `<?xml version="1.0" encoding="UTF-8" standalone="no"?>
...
${labeledNote('D', 5, 'W')}
    ...
    <measure number="2">
${labeledNote('E', 5, 'H')}
${labeledNote('F', 5, 'aug2')}
${labeledNote('G', 5, 'H', 1, 'sharp')}
${plainNote('A', 5)}
    </measure>
  </part>
</score-partwise>`
