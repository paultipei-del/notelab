...
export default function AdminRhythm() {
  ...
  async function handleUpload() {
    ...
    try {
      ...
      const { error: dbError } = await sb.from('rhythm_exercises').insert({
        title: title.trim(), category, difficulty,
        beats, beat_type: beatType, order_index: orderIndex, file_path: filePath,
      })
      if (dbError) throw new Error('Database: ' + dbError.message)
      setMessage({ text: `✓ "${title}" uploaded.`, ok: true })
  ...
  async function handleDelete(ex: RhythmExerciseMeta) {
    ...
    await sb.storage.from('rhythm-exercises').remove([ex.file_path])
  ...
  const inp: React.CSSProperties = { width: '100%', padding: '10px 14px', borderRadius: '10px', border: '1px solid #D3D1C7', fontFamily: F, fontSize: '14px', color: '#1A1A18', background: '#F5F2EC', outline: 'none', boxSizing: 'border-box' }

  return (
    <div style={{ minHeight: '100vh', background: '#F5F2EC', padding: '40px 32px' }}>
      <div style={{ maxWidth: '800px', margin: '0 auto' }}>
        <h1 style={{ fontFamily: SERIF, fontWeight: 300, fontSize: '32px', color: '#1A1A18', marginBottom: '4px' }}>Rhythm Library Admin</h1>
        <p style={{ fontFamily: F, fontSize: '13px', fontWeight: 300, color: '#888780', marginBottom: '40px' }}>Upload and manage rhythm exercises</p>

        <div style={{ background: 'white', borderRadius: '16px', border: '1px solid #D3D1C7', padding: '28px', marginBottom: '40px' }}>
          <p style={{ fontFamily: F, fontSize: '11px', fontWeight: 400, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#888780', marginBottom: '20px' }}>Upload Exercise</p>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '16px' }}>
            ...
            <div style={{ gridColumn: '1 / -1' }}>
              ...
              <input type="file" accept=".mxl,.xml,.musicxml" onChange={e => setFile(e.target.files?.[0] ?? null)} style={{ fontFamily: F, fontSize: '13px', color: '#1A1A18' }} />
              {file && <p style={{ fontFamily: F, fontSize: '12px', color: '#888780', marginTop: '4px' }}>{file.name}</p>}
            </div>
          </div>
          {message && <p style={{ fontFamily: F, fontSize: '13px', color: message.ok ? '#4CAF50' : '#E53935', marginBottom: '12px' }}>{message.text}</p>}
        ...
        <div>
          <p style={{ fontFamily: F, fontSize: '11px', fontWeight: 400, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#888780', marginBottom: '16px' }}>Library ({exercises.length})</p>
          {loading && <p style={{ fontFamily: F, fontSize: '13px', color: '#888780' }}>Loading…</p>}
          {!loading && exercises.length === 0 && <p style={{ fontFamily: F, fontSize: '13px', color: '#888780' }}>No exercises yet.</p>}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                {exs.map(ex => (
                  <div key={ex.id} style={{ display: 'flex', alignItems: 'center', gap: '10px', background: 'white', borderRadius: '10px', border: '1px solid #D3D1C7', padding: '12px 16px' }}>
                    ...
                    <span style={{ fontFamily: F, fontSize: '10px', color: '#D3D1C7', maxWidth: '160px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{ex.file_path}</span>
                    <button onClick={() => handleDelete(ex)}
                      style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#F09595', fontFamily: F, fontSize: '12px', padding: '4px 8px', borderRadius: '6px' }}>Delete</button>
                  </div>
                ))}
              </div>
            </div>
