  ...
  return (
    <div style={{ background: '#FDFAF3', borderRadius: '10px', border: open ? '1px solid #1A1A18' : '1px solid #DDD8CA', overflow: 'hidden', transition: 'border 0.15s' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '11px 14px', cursor: 'pointer' }}
        onClick={() => setOpen(o => !o)}>
        <span style={{ fontFamily: SERIF, fontSize: '15px', color: '#2A2318', flex: 1 }}>{ex.title}</span>
        <span style={{ fontFamily: F, fontSize: '11px', color: '#7A7060' }}>{ex.beats}/{ex.beat_type}</span>
        <span style={{ fontFamily: F, fontSize: '11px', color: '#7A7060' }}>D{ex.difficulty}</span>
        <span style={{ fontFamily: F, fontSize: '10px', color: '#B0AEA8' }}>Lv{ex.level} #{ex.order_index}</span>
        <span style={{ fontFamily: F, fontSize: '11px', color: open ? '#1A1A18' : '#7A7060' }}>{open ? '▲' : '▼'}</span>
      </div>
        ...
        <div style={{ borderTop: '1px solid #F0EDE8', padding: '16px 14px', background: '#FAFAF8' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
            <div style={{ gridColumn: '1 / -1' }}>
              <label style={{ fontFamily: F, fontSize: '11px', color: '#7A7060', display: 'block', marginBottom: '4px' }}>Title</label>
              <input value={fields.title} onChange={e => set('title', e.target.value)} style={smallInp} />
            </div>
            <div>
              <label style={{ fontFamily: F, fontSize: '11px', color: '#7A7060', display: 'block', marginBottom: '4px' }}>Category</label>
              <select value={fields.category} onChange={e => set('category', e.target.value)} style={smallInp}>
                {CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
              </select>
            </div>
            <div>
              <label style={{ fontFamily: F, fontSize: '11px', color: '#7A7060', display: 'block', marginBottom: '4px' }}>Category sort</label>
              <input type="number" value={fields.category_sort} onChange={e => set('category_sort', Number(e.target.value))} style={smallInp} />
            </div>
            <div>
              <label style={{ fontFamily: F, fontSize: '11px', color: '#7A7060', display: 'block', marginBottom: '4px' }}>Level</label>
              <input type="number" min={1} value={fields.level} onChange={e => set('level', Math.max(1, Number(e.target.value)))} style={smallInp} />
            </div>
            <div>
              <label style={{ fontFamily: F, fontSize: '11px', color: '#7A7060', display: 'block', marginBottom: '4px' }}>Order within level</label>
              <input type="number" min={0} value={fields.order_index} onChange={e => set('order_index', Number(e.target.value))} style={smallInp} />
            </div>
            <div>
              <label style={{ fontFamily: F, fontSize: '11px', color: '#7A7060', display: 'block', marginBottom: '4px' }}>Difficulty</label>
            ...
            <div>
              <label style={{ fontFamily: F, fontSize: '11px', color: '#7A7060', display: 'block', marginBottom: '4px' }}>Time Signature</label>
            ...
            <div>
              <label style={{ fontFamily: F, fontSize: '11px', color: '#7A7060', display: 'block', marginBottom: '4px' }}>Program slug</label>
              <input value={fields.program_slug} onChange={e => set('program_slug', e.target.value)} style={smallInp} />
            </div>
            <div>
              <label style={{ fontFamily: F, fontSize: '11px', color: '#7A7060', display: 'block', marginBottom: '4px' }}>Program sort</label>
          ...
          <div style={{ display: 'flex', gap: '8px', justifyContent: 'space-between' }}>
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              <button type="button" onClick={() => { setOpen(false); setErr(null) }}
                style={{ background: 'none', border: '1px solid #DDD8CA', borderRadius: '8px', color: '#7A7060', fontFamily: F, fontSize: '12px', padding: '6px 14px', cursor: 'pointer' }}>
                Cancel
              </button>
              <button type="button" onClick={handleSave} disabled={saving}
                style={{ background: saving ? '#DDD8CA' : '#1A1A18', border: 'none', borderRadius: '8px', color: 'white', fontFamily: F, fontSize: '12px', padding: '6px 18px', cursor: saving ? 'default' : 'pointer' }}>
                {saving ? 'Saving…' : 'Save'}
              </button>
            </div>
          </div>
...
export default function AdminRhythm() {
  ...
  const inp: React.CSSProperties = {
    ...
    border: '1px solid #DDD8CA', fontFamily: F, fontSize: '14px',
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F2EDDF', padding: '40px 32px' }}>
      <div style={{ maxWidth: '800px', margin: '0 auto' }}>
        ...
        <p style={{ fontFamily: F, fontSize: '13px', fontWeight: 300, color: '#7A7060', marginBottom: '40px' }}>
        ...
        <div style={{ background: '#FDFAF3', borderRadius: '16px', border: '1px solid #DDD8CA', padding: '28px', marginBottom: '40px' }}>
          <p style={{ fontFamily: F, fontSize: '11px', fontWeight: 400, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#7A7060', marginBottom: '20px' }}>
            Upload Exercise
          </p>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '16px' }}>
            <div style={{ gridColumn: '1 / -1' }}>
              <label style={{ fontFamily: F, fontSize: '11px', color: '#7A7060', display: 'block', marginBottom: '6px' }}>Title</label>
              <input value={title} onChange={e => setTitle(e.target.value)} placeholder="e.g. Quarter Notes #1" style={inp} />
            </div>
            <div>
              <label style={{ fontFamily: F, fontSize: '11px', color: '#7A7060', display: 'block', marginBottom: '6px' }}>Program slug</label>
              <input value={programSlug} onChange={e => setProgramSlug(e.target.value)} placeholder="core" style={inp} />
            </div>
            <div>
              <label style={{ fontFamily: F, fontSize: '11px', color: '#7A7060', display: 'block', marginBottom: '6px' }}>Program sort</label>
              <input type="number" value={programSort} onChange={e => setProgramSort(Number(e.target.value))} style={inp} />
            </div>
            <div>
              <label style={{ fontFamily: F, fontSize: '11px', color: '#7A7060', display: 'block', marginBottom: '6px' }}>Category</label>
              <select value={category} onChange={e => setCategory(e.target.value)} style={inp}>
                {CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
              </select>
            </div>
            <div>
              <label style={{ fontFamily: F, fontSize: '11px', color: '#7A7060', display: 'block', marginBottom: '6px' }}>Category sort</label>
              <input type="number" value={categorySort} onChange={e => setCategorySort(Number(e.target.value))} style={inp} />
            </div>
            <div>
              <label style={{ fontFamily: F, fontSize: '11px', color: '#7A7060', display: 'block', marginBottom: '6px' }}>Level</label>
              <input type="number" value={level} onChange={e => setLevel(Math.max(1, Number(e.target.value)))} min={1} style={inp} />
            </div>
            <div>
              <label style={{ fontFamily: F, fontSize: '11px', color: '#7A7060', display: 'block', marginBottom: '6px' }}>Difficulty</label>
              <select value={difficulty} onChange={e => setDifficulty(Number(e.target.value))} style={inp}>
                {DIFFICULTIES.map(d => <option key={d} value={d}>{d} — {DIFFICULTY_LABEL[d]}</option>)}
              </select>
            </div>
            <div>
              <label style={{ fontFamily: F, fontSize: '11px', color: '#7A7060', display: 'block', marginBottom: '6px' }}>Time Signature</label>
            ...
            <div>
              <label style={{ fontFamily: F, fontSize: '11px', color: '#7A7060', display: 'block', marginBottom: '6px' }}>Order within level</label>
              <input type="number" value={orderIndex} onChange={e => setOrderIndex(Number(e.target.value))} min={0} style={inp} />
            </div>
            <div style={{ gridColumn: '1 / -1' }}>
              <label style={{ fontFamily: F, fontSize: '11px', color: '#7A7060', display: 'block', marginBottom: '6px' }}>MXL File</label>
              <input type="file" accept=".mxl,.xml,.musicxml"
                onChange={e => setFile(e.target.files?.[0] ?? null)}
                style={{ fontFamily: F, fontSize: '13px', color: '#2A2318' }} />
              {file && <p style={{ fontFamily: F, fontSize: '12px', color: '#7A7060', marginTop: '4px' }}>{file.name} ({(file.size / 1024).toFixed(1)} KB)</p>}
            </div>
          </div>
          ...
          <button onClick={handleUpload} disabled={uploading || !file || !title.trim()}
            style={{ background: uploading || !file || !title.trim() ? '#DDD8CA' : '#1A1A18', color: 'white', border: 'none', borderRadius: '10px', padding: '10px 28px', fontFamily: F, fontSize: '13px', fontWeight: 300, cursor: uploading || !file || !title.trim() ? 'default' : 'pointer' }}>
        ...
        {/* Library */}
        <div>
          <p style={{ fontFamily: F, fontSize: '11px', fontWeight: 400, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#7A7060', marginBottom: '16px' }}>
            Library ({exercises.length})
          </p>
          {loading && <p style={{ fontFamily: F, fontSize: '13px', color: '#7A7060' }}>Loading…</p>}
          {!loading && exercises.length === 0 && (
            <p style={{ fontFamily: F, fontSize: '13px', color: '#7A7060' }}>No exercises yet.</p>
          )}
          {!loading && libraryTree.map(program => (
            <div key={program.slug} style={{ marginBottom: '28px' }}>
              <p style={{ fontFamily: SERIF, fontSize: '20px', fontWeight: 300, color: '#2A2318', marginBottom: '12px' }}>
                {rhythmProgramTitle(program.slug)} <span style={{ fontFamily: F, fontSize: '12px', color: '#7A7060' }}>({program.slug})</span>
              </p>
                ...
                <div key={`${program.slug}::${cat.name}`} style={{ marginBottom: '20px', marginLeft: '8px' }}>
                  ...
                  {cat.levels.map(lvl => (
                    <div key={lvl.level} style={{ marginBottom: '12px', marginLeft: '12px' }}>
                      <p style={{ fontFamily: F, fontSize: '11px', fontWeight: 600, letterSpacing: '0.06em', textTransform: 'uppercase', color: '#7A7060', marginBottom: '6px' }}>
                        Level {lvl.level}
                      </p>
