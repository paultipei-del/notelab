  ...
  return (
    <div style={{ background: '#FDFAF3', borderRadius: '10px', border: open ? '1px solid #1A1A18' : '1px solid #DDD8CA', overflow: 'hidden', transition: 'border 0.15s' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '11px 14px', cursor: 'pointer' }}
        onClick={() => setOpen(o => !o)}>
        <span style={{ fontFamily: SERIF, fontSize: 'var(--nl-text-body)', color: '#2A2318', flex: 1 }}>{ex.title}</span>
        <span style={{ fontFamily: F, fontSize: 'var(--nl-text-compact)', color: '#7A7060' }}>{ex.beats}/{ex.beat_type}</span>
        <span style={{ fontFamily: F, fontSize: 'var(--nl-text-compact)', color: '#7A7060' }}>D{ex.difficulty}</span>
        <span style={{ fontFamily: F, fontSize: 'var(--nl-text-badge)', color: '#B0AEA8' }}>Lv{ex.level} #{ex.order_index}</span>
        <span style={{ fontFamily: F, fontSize: 'var(--nl-text-compact)', color: open ? '#1A1A18' : '#7A7060' }}>{open ? '▲' : '▼'}</span>
      </div>
        ...
        <div style={{ borderTop: '1px solid #F0EDE8', padding: '16px 14px', background: '#FAFAF8' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
            <div style={{ gridColumn: '1 / -1' }}>
              <label style={{ fontFamily: F, fontSize: 'var(--nl-text-compact)', color: '#7A7060', display: 'block', marginBottom: '4px' }}>Title</label>
              <input value={fields.title} onChange={e => set('title', e.target.value)} style={smallInp} />
            </div>
            <div>
              <label style={{ fontFamily: F, fontSize: 'var(--nl-text-compact)', color: '#7A7060', display: 'block', marginBottom: '4px' }}>Category</label>
              <select value={fields.category} onChange={e => set('category', e.target.value)} style={smallInp}>
                {CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
              </select>
            </div>
            <div>
              <label style={{ fontFamily: F, fontSize: 'var(--nl-text-compact)', color: '#7A7060', display: 'block', marginBottom: '4px' }}>Category sort</label>
              <input type="number" value={fields.category_sort} onChange={e => set('category_sort', Number(e.target.value))} style={smallInp} />
            </div>
            <div>
              <label style={{ fontFamily: F, fontSize: 'var(--nl-text-compact)', color: '#7A7060', display: 'block', marginBottom: '4px' }}>Level</label>
              <input type="number" min={1} value={fields.level} onChange={e => set('level', Math.max(1, Number(e.target.value)))} style={smallInp} />
            </div>
            <div>
              <label style={{ fontFamily: F, fontSize: 'var(--nl-text-compact)', color: '#7A7060', display: 'block', marginBottom: '4px' }}>Order within level</label>
              <input type="number" min={0} value={fields.order_index} onChange={e => set('order_index', Number(e.target.value))} style={smallInp} />
            </div>
            <div>
              <label style={{ fontFamily: F, fontSize: 'var(--nl-text-compact)', color: '#7A7060', display: 'block', marginBottom: '4px' }}>Difficulty</label>
            ...
            <div>
              <label style={{ fontFamily: F, fontSize: 'var(--nl-text-compact)', color: '#7A7060', display: 'block', marginBottom: '4px' }}>Time Signature</label>
              <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                ...
                <span style={{ fontFamily: SERIF, fontSize: '18px', color: '#7A7060' }}>/</span>
            ...
            <div>
              <label style={{ fontFamily: F, fontSize: 'var(--nl-text-compact)', color: '#7A7060', display: 'block', marginBottom: '4px' }}>Program slug</label>
              <input value={fields.program_slug} onChange={e => set('program_slug', e.target.value)} style={smallInp} />
            </div>
            <div>
              <label style={{ fontFamily: F, fontSize: 'var(--nl-text-compact)', color: '#7A7060', display: 'block', marginBottom: '4px' }}>Program sort</label>
              <input type="number" value={fields.program_sort} onChange={e => set('program_sort', Number(e.target.value))} style={smallInp} />
            </div>
          </div>

          {err && <p style={{ fontFamily: F, fontSize: 'var(--nl-text-compact)', color: '#E53935', marginBottom: '10px' }}>{err}</p>}

          <div style={{ display: 'flex', gap: '8px', justifyContent: 'space-between' }}>
            <button type="button" onClick={onDelete}
              style={{ background: 'none', border: '1px solid #F09595', borderRadius: '8px', color: '#E53935', fontFamily: F, fontSize: 'var(--nl-text-compact)', padding: '6px 14px', cursor: 'pointer' }}>
              Delete exercise
            </button>
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F2EDDF', padding: '40px 32px' }}>
      <div style={{ maxWidth: '800px', margin: '0 auto' }}>
        ...
        <p style={{ fontFamily: F, fontSize: 'var(--nl-text-meta)', fontWeight: 400, color: '#7A7060', marginBottom: '40px' }}>
        ...
        <div style={{ background: '#FDFAF3', borderRadius: '16px', border: '1px solid #DDD8CA', padding: '28px', marginBottom: '40px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '16px' }}>
            <div style={{ gridColumn: '1 / -1' }}>
              <label style={{ fontFamily: F, fontSize: 'var(--nl-text-compact)', color: '#7A7060', display: 'block', marginBottom: '6px' }}>Title</label>
              <input value={title} onChange={e => setTitle(e.target.value)} placeholder="e.g. Quarter Notes #1" style={inp} />
            </div>
            <div>
              <label style={{ fontFamily: F, fontSize: 'var(--nl-text-compact)', color: '#7A7060', display: 'block', marginBottom: '6px' }}>Program slug</label>
              <input value={programSlug} onChange={e => setProgramSlug(e.target.value)} placeholder="core" style={inp} />
            </div>
            <div>
              <label style={{ fontFamily: F, fontSize: 'var(--nl-text-compact)', color: '#7A7060', display: 'block', marginBottom: '6px' }}>Program sort</label>
              <input type="number" value={programSort} onChange={e => setProgramSort(Number(e.target.value))} style={inp} />
            </div>
            <div>
              <label style={{ fontFamily: F, fontSize: 'var(--nl-text-compact)', color: '#7A7060', display: 'block', marginBottom: '6px' }}>Category</label>
              <select value={category} onChange={e => setCategory(e.target.value)} style={inp}>
                {CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
              </select>
            </div>
            <div>
              <label style={{ fontFamily: F, fontSize: 'var(--nl-text-compact)', color: '#7A7060', display: 'block', marginBottom: '6px' }}>Category sort</label>
              <input type="number" value={categorySort} onChange={e => setCategorySort(Number(e.target.value))} style={inp} />
            </div>
            <div>
              <label style={{ fontFamily: F, fontSize: 'var(--nl-text-compact)', color: '#7A7060', display: 'block', marginBottom: '6px' }}>Level</label>
              <input type="number" value={level} onChange={e => setLevel(Math.max(1, Number(e.target.value)))} min={1} style={inp} />
            </div>
            <div>
              <label style={{ fontFamily: F, fontSize: 'var(--nl-text-compact)', color: '#7A7060', display: 'block', marginBottom: '6px' }}>Difficulty</label>
              <select value={difficulty} onChange={e => setDifficulty(Number(e.target.value))} style={inp}>
                {DIFFICULTIES.map(d => <option key={d} value={d}>{d} — {DIFFICULTY_LABEL[d]}</option>)}
              </select>
            </div>
            <div>
              <label style={{ fontFamily: F, fontSize: 'var(--nl-text-compact)', color: '#7A7060', display: 'block', marginBottom: '6px' }}>Time Signature</label>
              <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                <select value={beats} onChange={e => setBeats(Number(e.target.value))} style={{ ...inp, flex: 1 }}>
                  {[2,3,4,5,6,9,12].map(n => <option key={n} value={n}>{n}</option>)}
                </select>
                <span style={{ fontFamily: SERIF, fontSize: '20px', color: '#7A7060' }}>/</span>
            ...
            <div>
              <label style={{ fontFamily: F, fontSize: 'var(--nl-text-compact)', color: '#7A7060', display: 'block', marginBottom: '6px' }}>Order within level</label>
              <input type="number" value={orderIndex} onChange={e => setOrderIndex(Number(e.target.value))} min={0} style={inp} />
            </div>
            <div style={{ gridColumn: '1 / -1' }}>
              <label style={{ fontFamily: F, fontSize: 'var(--nl-text-compact)', color: '#7A7060', display: 'block', marginBottom: '6px' }}>MXL File</label>
              <input type="file" accept=".mxl,.xml,.musicxml"
                onChange={e => setFile(e.target.files?.[0] ?? null)}
                style={{ fontFamily: F, fontSize: 'var(--nl-text-meta)', color: '#2A2318' }} />
              {file && <p style={{ fontFamily: F, fontSize: 'var(--nl-text-compact)', color: '#7A7060', marginTop: '4px' }}>{file.name} ({(file.size / 1024).toFixed(1)} KB)</p>}
            </div>
          </div>

          {message && (
            <p style={{ fontFamily: F, fontSize: 'var(--nl-text-meta)', color: message.ok ? '#4CAF50' : '#E53935', marginBottom: '12px' }}>
              {message.text}
            </p>
          )}
        ...
        {/* Library */}
        <div>
          <p style={{ fontFamily: F, fontSize: 'var(--nl-text-compact)', fontWeight: 400, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#7A7060', marginBottom: '16px' }}>
            Library ({exercises.length})
          </p>
          {loading && <p style={{ fontFamily: F, fontSize: 'var(--nl-text-meta)', color: '#7A7060' }}>Loading…</p>}
          {!loading && exercises.length === 0 && (
            <p style={{ fontFamily: F, fontSize: 'var(--nl-text-meta)', color: '#7A7060' }}>No exercises yet.</p>
          )}
          {!loading && libraryTree.map(program => (
            <div key={program.slug} style={{ marginBottom: '28px' }}>
              <p style={{ fontFamily: SERIF, fontSize: '20px', fontWeight: 300, color: '#2A2318', marginBottom: '12px' }}>
                {rhythmProgramTitle(program.slug)} <span style={{ fontFamily: F, fontSize: 'var(--nl-text-compact)', color: '#7A7060' }}>({program.slug})</span>
              </p>
              {program.categories.map(cat => (
                <div key={`${program.slug}::${cat.name}`} style={{ marginBottom: '20px', marginLeft: '8px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
                    <p style={{ fontFamily: SERIF, fontSize: '17px', fontWeight: 300, color: '#2A2318', margin: 0 }}>{cat.name}</p>
                    <span style={{ fontFamily: F, fontSize: 'var(--nl-text-badge)', color: '#B0AEA8' }}>cat_sort={cat.category_sort}</span>
                    <button type="button" onClick={() => handleDeleteCategoryScope(program.slug, cat.name)}
                      style={{ background: 'none', border: '1px solid #F09595', borderRadius: '8px', color: '#E53935', fontFamily: F, fontSize: 'var(--nl-text-compact)', padding: '2px 8px', cursor: 'pointer' }}>
                      Delete category
                    </button>
                  </div>
                  {cat.levels.map(lvl => (
                    <div key={lvl.level} style={{ marginBottom: '12px', marginLeft: '12px' }}>
                      <p style={{ fontFamily: F, fontSize: 'var(--nl-text-compact)', fontWeight: 600, letterSpacing: '0.06em', textTransform: 'uppercase', color: '#7A7060', marginBottom: '6px' }}>
                        Level {lvl.level}
                      </p>
