  ...
  return (
    <div style={{ minHeight: '100vh', background: '#2C2A27' }}>
      <div style={{ maxWidth: '680px', margin: '0 auto', padding: '24px 32px 80px' }}>
        <button onClick={() => router.back()} style={{ background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'var(--font-jost), sans-serif', fontSize: '13px', fontWeight: 300, color: '#C4C0B8', padding: 0, marginBottom: '32px', display: 'block' }}>← Back</button>
        <p style={{ fontFamily: 'var(--font-jost), sans-serif', fontWeight: 300, fontSize: '13px', color: '#C4C0B8', marginBottom: '24px' }}>
          ...
          {GRAND_LEVELS.map((deck, i) => {
            ...
            return (
              <button key={deck.id} onClick={() => !locked && router.push('/study/' + deck.id)}
                ...
                <div>
                  <p style={{ fontFamily: 'var(--font-cormorant), serif', fontWeight: 400, fontSize: '18px', color: '#F7F4EF', marginBottom: '2px' }}>{deck.title}</p>
                  <p style={{ fontFamily: 'var(--font-jost), sans-serif', fontWeight: 300, fontSize: '12px', color: '#C4C0B8' }}>{deck.description}</p>
                </div>
                {locked ? <span style={{ fontSize: '14px', color: '#484542' }}>🔒</span> : <span style={{ fontSize: '13px', color: '#484542' }}>→</span>}
              </button>
            )
          })}
        </div>
