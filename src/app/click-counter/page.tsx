...
export default function ClickCounterPage() {
  ...
  }, [sessionDone, setsGoal, showCompletion])
  ...
  const increment = useCallback(() => {
    ...
    flash()
  }, [flash, logSet, pendingLog, repsGoal, sessionDone, startTimer])
      ...
      <div style={{ width: '100%', maxWidth: 900, margin: '0 auto', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        ...
        <div style={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '0.6rem 2rem 1.2rem' }}>
          ...
          <div style={{ display: 'flex', marginTop: '0.5rem', width: '100%', justifyContent: 'center' }}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.25rem', width: 160 }}>
              <span style={{ fontFamily: SERIF, fontSize: '1.5rem', color: '#2A2318' }}>{sessionTotal}</span>
              <span style={{ fontFamily: F, fontSize: 10, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#7A7060' }}>Total reps</span>
              <span style={{ display: 'block', height: '1.4rem' }} />
              <span style={{ fontFamily: F, fontSize: 12, letterSpacing: '0.18em', textTransform: 'uppercase', color: '#7A7060', whiteSpace: 'nowrap' }}>Reps target</span>
              <input value={repsTarget} onChange={(e) => setRepsTarget(e.target.value === '' ? '' : Math.max(0, Number(e.target.value) || 0))} type="number" min={0} placeholder="—" style={{ fontFamily: SERIF, fontSize: '1.3rem', color: '#2A2318', background: 'transparent', border: 'none', borderBottom: '1px solid rgba(42,35,24,0.3)', width: 70, textAlign: 'center', outline: 'none', padding: '2px 0' }} />
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.25rem', width: 160 }}>
              <span style={{ fontFamily: SERIF, fontSize: '1.5rem', color: '#2A2318' }}>{sets.length}</span>
              <span style={{ fontFamily: F, fontSize: 10, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#7A7060' }}>Sets</span>
              <span style={{ display: 'block', height: '1.4rem' }} />
              <span style={{ fontFamily: F, fontSize: 12, letterSpacing: '0.18em', textTransform: 'uppercase', color: '#7A7060', whiteSpace: 'nowrap' }}>Sets target</span>
              <input value={setsTarget} onChange={(e) => setSetsTarget(e.target.value === '' ? '' : Math.max(0, Number(e.target.value) || 0))} type="number" min={0} placeholder="—" style={{ fontFamily: SERIF, fontSize: '1.3rem', color: '#2A2318', background: 'transparent', border: 'none', borderBottom: '1px solid rgba(42,35,24,0.3)', width: 70, textAlign: 'center', outline: 'none', padding: '2px 0' }} />
            </div>
          </div>
        ...
        <div style={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '1rem 2rem 0.5rem', gap: '1rem' }}>
          ...
          <div style={{ fontFamily: F, fontSize: 11, letterSpacing: '0.2em', textTransform: 'uppercase', color: '#7A7060' }}>Tap or press Space</div>
          ...
          <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center', justifyContent: 'center' }}>
            <button
              ...
              onClick={decrement}
              style={{
                width: 40, height: 40, borderRadius: 20, border: '1px solid rgba(42,35,24,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center',
                fontSize: 18, cursor: sessionDone ? 'default' : 'pointer', background: touchMinusActive ? '#2A2318' : 'transparent', color: touchMinusActive ? '#F2EDDF' : '#2A2318',
                userSelect: 'none', opacity: sessionDone ? 0.35 : 1, pointerEvents: sessionDone ? 'none' : undefined,
              }}
            >
              −
            </button>
