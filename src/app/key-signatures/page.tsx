      ...
      {keys12.map((k, i) => {
        ...
        return (
          <path key={'ma-'+k.major} d={wedgePath(innerR, outerR, i)}
            fill={(isSelected || isAltSelected) ? '#1A1A18' : '#F5F2EC'}
            stroke="white" strokeWidth="2"
            onClick={() => onSelect(isAltSelected ? k.alt! : k.major)} style={{ cursor: 'pointer' }} />
        )
      })}
      {/* Center circle */}
      <circle cx={cx} cy={cy} r={minorR - 2} fill="white" stroke="#D3D1C7" strokeWidth="1" />
      {/* Minor text labels */}
      {keys12.map((k, i) => {
        ...
        return (
          <text key={'mit-'+k.major} x={tx} y={ty} textAnchor="middle" dominantBaseline="middle"
            fontSize="10" fontFamily={F} fill={isSelected ? 'white' : '#888780'} fontWeight="300"
            onClick={() => onSelect(k.major)} style={{ cursor: 'pointer' }}>
            {displayMinor}
          </text>
        )
            ...
            {k.alt ? (
              <>
                ...
                <line x1={tx - 8} y1={ty} x2={tx + 8} y2={ty}
                  stroke={(isSelected || isAltSelected) ? 'rgba(255,255,255,0.4)' : '#D3D1C7'}
                  strokeWidth="0.5" />
                <text x={tx} y={ty + 7} textAnchor="middle" dominantBaseline="middle"
                  fontSize="12" fontFamily={SERIF}
                  fill={(isSelected || isAltSelected) ? 'white' : '#1A1A18'}
        ...
        {whiteKeys.map((k, i) => {
          ...
          return (
            <div key={k.note+k.oct} style={{
              ...
              {k.note === 'C' && <span style={{ fontSize: '8px', color: '#D3D1C7', fontFamily: F }}>C{k.oct}</span>}
      ...
      <div style={{ maxWidth: '960px', margin: '0 auto', padding: '24px 24px 80px' }}>
        ...
        <div style={{ display: 'flex', gap: '32px', flexWrap: 'wrap' as const }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column' as const, alignItems: 'center', gap: '16px' }}>
            <p style={{ fontFamily: F, fontSize: '11px', fontWeight: 300, letterSpacing: '0.12em', textTransform: 'uppercase' as const, color: '#888780' }}>Circle of Fifths</p>
          ...
          <div style={{ flex: 1, minWidth: '300px', display: 'flex', flexDirection: 'column' as const, gap: '16px' }}>
            ...
            {/* Key name + info */}
            <div style={{ background: 'white', borderRadius: '16px', border: '1px solid #D3D1C7', padding: '24px' }}>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: '12px', marginBottom: '8px' }}>
                <h2 style={{ fontFamily: SERIF, fontSize: '36px', fontWeight: 300, color: '#1A1A18' }}>{selectedKey} Major</h2>
              ...
              <div style={{ display: 'flex', gap: '24px', flexWrap: 'wrap' as const }}>
                <div>
                  <p style={{ fontFamily: F, fontSize: '10px', fontWeight: 300, color: '#888780', letterSpacing: '0.1em', textTransform: 'uppercase' as const, marginBottom: '4px' }}>Relative Minor</p>
                  <p style={{ fontFamily: SERIF, fontSize: '20px', fontWeight: 300, color: '#1A1A18' }}>{keyInfo.relativeMinor} minor</p>
                </div>
                <div>
                  <p style={{ fontFamily: F, fontSize: '10px', fontWeight: 300, color: '#888780', letterSpacing: '0.1em', textTransform: 'uppercase' as const, marginBottom: '4px' }}>
                    {keyInfo.sharps > 0 ? 'Sharps' : keyInfo.flats > 0 ? 'Flats' : 'Notes'}
                  </p>
                  <p style={{ fontFamily: SERIF, fontSize: '16px', fontWeight: 300, color: '#1A1A18' }}>
                ...
                <div style={{ display: 'flex', gap: '6px' }}>
                  {(['treble','bass','both'] as const).map(c => (
                    <button key={c} onClick={() => setClef(c)}
                      style={{ padding: '4px 10px', borderRadius: '12px', border: '1px solid ' + (clef === c ? '#1A1A18' : '#D3D1C7'), background: clef === c ? '#1A1A18' : 'white', color: clef === c ? 'white' : '#888780', fontFamily: F, fontSize: '11px', fontWeight: 300, cursor: 'pointer' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                ...
                <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' as const }}>
                  <button onClick={() => { setShowScale(true); setShowRelativeOnPiano(false); playScale() }}
                    style={{ padding: '4px 12px', borderRadius: '12px', border: '1px solid ' + (!showRelativeOnPiano && showScale ? '#1A1A18' : '#D3D1C7'), background: !showRelativeOnPiano && showScale ? '#1A1A18' : 'white', color: !showRelativeOnPiano && showScale ? 'white' : '#888780', fontFamily: F, fontSize: '11px', fontWeight: 300, cursor: 'pointer' }}>
                    ▶ {selectedKey} Major
                  </button>
                  <button onClick={() => { setShowScale(true); setShowRelativeOnPiano(true); playRelativeMinor() }}
                    style={{ padding: '4px 12px', borderRadius: '12px', border: '1px solid ' + (showRelativeOnPiano ? '#1A1A18' : '#D3D1C7'), background: showRelativeOnPiano ? '#1A1A18' : 'white', color: showRelativeOnPiano ? 'white' : '#888780', fontFamily: F, fontSize: '11px', fontWeight: 300, cursor: 'pointer' }}>
                    ▶ {keyInfo.relativeMinor} minor
                  </button>
                  <button onClick={() => setShowScale(!showScale)}
                    style={{ padding: '4px 12px', borderRadius: '12px', border: '1px solid #D3D1C7', background: 'white', color: '#888780', fontFamily: F, fontSize: '11px', fontWeight: 300, cursor: 'pointer' }}>
                    {showScale ? 'Hide' : 'Show'}
                  </button>
                </div>
              </div>
