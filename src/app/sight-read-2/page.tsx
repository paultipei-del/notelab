  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F2EDDF' }}>
      <div style={{ maxWidth: '600px', margin: '0 auto', padding: '24px 24px', display: 'flex', flexDirection: 'column' as const, gap: '16px' }}>
        <button onClick={() => router.back()} style={{ background: 'none', border: 'none', cursor: 'pointer', fontFamily: F, fontSize: '13px', fontWeight: 300, color: '#7A7060', padding: 0, marginBottom: '16px', display: 'block', alignSelf: 'flex-start' }}>← Back</button>
        {[
          ...
          <Link key={href} href={href} style={{ textDecoration: 'none' }}>
            <div style={{ background: '#FDFAF3', border: '1px solid #DDD8CA', borderRadius: '16px', padding: '24px 28px', cursor: 'pointer' }}>
              ...
              <p style={{ fontFamily: F, fontSize: '13px', fontWeight: 300, color: '#7A7060' }}>{desc}</p>
            </div>
          </Link>
        ))}
      </div>
