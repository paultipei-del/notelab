...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div style={{ marginBottom: '40px' }}>
      <h2 style={{ fontFamily: SERIF, fontWeight: 300, fontSize: '26px', color: '#F7F4EF', marginBottom: '20px', letterSpacing: '0.01em' }}>{title}</h2>
      {children}
    </div>
  )
}
...
function Row({ label, value, children }: { label: string; value?: string; children?: React.ReactNode }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 0', borderBottom: '1px solid #EDE8DF' }}>
      <span style={{ fontFamily: F, fontSize: '13px', fontWeight: 300, color: '#C4C0B8', minWidth: '120px' }}>{label}</span>
      <span style={{ fontFamily: F, fontSize: '14px', fontWeight: 300, color: '#F7F4EF', flex: 1, textAlign: 'right' as const }}>{value}</span>
      {children}
    </div>
  )
}
    ...
    <Section title="Profile">
      <Card>
        ...
        <div style={{ paddingBottom: '20px', marginBottom: '4px', borderBottom: '1px solid #EDE8DF' }}>
          <label style={{ fontFamily: F, fontSize: '11px', fontWeight: 400, letterSpacing: '0.1em', textTransform: 'uppercase' as const, color: '#C4C0B8', display: 'block', marginBottom: '8px' }}>
            Display name
          </label>
          <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
            <input
              ...
              style={{
                ...
                fontFamily: F, fontSize: '14px', fontWeight: 300, color: '#F7F4EF',
                outline: 'none',
              }}
            />
          ...
          <p style={{ fontFamily: F, fontSize: '11px', fontWeight: 300, color: '#C4C0B8', margin: '6px 0 0' }}>
...
function SubscriptionSection({ userId }: { userId: string }) {
  ...
  return (
    <Section title="Subscription">
      <Card>
        {loading ? (
          <p style={{ fontFamily: F, fontSize: '13px', fontWeight: 300, color: '#C4C0B8' }}>Loading…</p>
        ) : (
          <>
            <div style={{ borderBottom: '1px solid #EDE8DF' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 0', borderBottom: '1px solid #EDE8DF' }}>
                <div>
                  <p style={{ fontFamily: F, fontSize: '14px', fontWeight: 300, color: '#F7F4EF', margin: '0 0 2px' }}>CM Collection Bundle</p>
                  <p style={{ fontFamily: F, fontSize: '12px', fontWeight: 300, color: '#C4C0B8', margin: 0 }}>All Certificate of Merit levels, Prep – Advanced</p>
                </div>
                {cmUnlocked ? <Pill label="Active" /> : <Pill label="Not purchased" color="#C4C0B8" bg="#EDE8DF" />}
              </div>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 0' }}>
                <div>
                  <p style={{ fontFamily: F, fontSize: '14px', fontWeight: 300, color: '#F7F4EF', margin: '0 0 2px' }}>Pro Subscription</p>
                  <p style={{ fontFamily: F, fontSize: '12px', fontWeight: 300, color: '#C4C0B8', margin: 0 }}>All content + future features</p>
                </div>
                {proUnlocked ? <Pill label="Active" /> : <Pill label="Not active" color="#C4C0B8" bg="#EDE8DF" />}
              </div>
            </div>
            ...
            {(cmUnlocked || proUnlocked) && (
              <p style={{ fontFamily: F, fontSize: '12px', fontWeight: 300, color: '#C4C0B8', marginTop: '16px', marginBottom: 0 }}>
  ...
  return (
    <Section title="Progress">
      {loading ? (
        ...
        <Card>
          <p style={{ fontFamily: F, fontSize: '14px', fontWeight: 300, color: '#C4C0B8', margin: 0 }}>No study progress yet. Start a flashcard deck to track your progress here.</p>
        </Card>
      ) : (
        <>
              ...
              return (
                <Card key={deck.id} style={{ padding: '18px 24px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px' }}>
                    <div>
                      <p style={{ fontFamily: SERIF, fontSize: '18px', fontWeight: 400, color: '#F7F4EF', margin: '0 0 2px' }}>{deck.title}</p>
                      <p style={{ fontFamily: F, fontSize: '12px', fontWeight: 300, color: '#C4C0B8', margin: 0 }}>
                  ...
                  <div style={{ height: '4px', background: '#EDE8DF', borderRadius: '2px', overflow: 'hidden' }}>
                    <div style={{ height: '100%', width: pct + '%', background: '#BA7517', borderRadius: '2px', transition: 'width 0.4s' }} />
                  </div>
                </Card>
              )
            })}
  ...
  return (
    <Section title="My Decks">
      {loading ? (
        ...
        <Card>
          <p style={{ fontFamily: F, fontSize: '14px', fontWeight: 300, color: '#C4C0B8', margin: 0 }}>
            No custom decks yet.{' '}
        ...
        <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '10px' }}>
          {decks.map(deck => (
            <Card key={deck.id} style={{ padding: '18px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div>
                <p style={{ fontFamily: SERIF, fontSize: '18px', fontWeight: 400, color: '#F7F4EF', margin: '0 0 2px' }}>{deck.title}</p>
                <p style={{ fontFamily: F, fontSize: '12px', fontWeight: 300, color: '#C4C0B8', margin: 0 }}>{deck.cards.length} cards · {deck.tag}</p>
              </div>
  ...
  if (loading || !user) {
    return (
      <div style={{ minHeight: '100vh', background: '#2C2A27', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <p style={{ fontFamily: F, fontWeight: 300, color: '#C4C0B8' }}>Loading…</p>
      </div>
    )
  }
    ...
    <div style={{ minHeight: '100vh', background: '#2C2A27' }}>
      <div style={{ maxWidth: '960px', margin: '0 auto', padding: '48px 32px 80px', display: 'flex', gap: '40px', alignItems: 'flex-start' }}>
        ...
        <aside style={{ width: '180px', flexShrink: 0, position: 'sticky' as const, top: '80px' }}>
          <p style={{ fontFamily: F, fontSize: '11px', fontWeight: 400, letterSpacing: '0.12em', textTransform: 'uppercase' as const, color: '#C4C0B8', marginBottom: '12px' }}>Account</p>
          <nav style={{ display: 'flex', flexDirection: 'column' as const, gap: '2px' }}>
            {SECTIONS.map(s => (
              <button key={s.id} onClick={() => setActive(s.id)} style={{
                ...
                color: active === s.id ? '#1A1A18' : '#C4C0B8',
          ...
          <div style={{ marginTop: '24px', paddingTop: '20px', borderTop: '1px solid #484542' }}>
            <Link href="/" style={{ textDecoration: 'none' }}>
              <span style={{ fontFamily: F, fontSize: '12px', fontWeight: 300, color: '#C4C0B8' }}>← Back to app</span>
            </Link>
          </div>
        </aside>
