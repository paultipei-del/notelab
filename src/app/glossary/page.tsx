...
export default function Glossary() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F2EDDF' }}>
      {/* Search + filters */}
      <div style={{ background: '#FDFAF3', borderBottom: '1px solid #DDD8CA', padding: '12px 32px', display: 'flex', gap: '12px', flexWrap: 'wrap' as const, alignItems: 'center', position: 'sticky' as const, top: '56px', zIndex: 9 }}>
        <button onClick={() => router.back()} style={{ background: 'none', border: 'none', cursor: 'pointer', fontFamily: F, fontSize: '13px', fontWeight: 300, color: '#7A7060', padding: 0, flexShrink: 0 }}>← Back</button>
        <input
          value={query}
          onChange={e => setQuery(e.target.value)}
          placeholder="Search terms or definitions…"
          style={{ flex: 1, minWidth: '200px', padding: '10px 16px', borderRadius: '10px', border: '1px solid #DDD8CA', background: '#F2EDDF', fontFamily: F, fontSize: '13px', fontWeight: 300, color: '#2A2318', outline: 'none' }}
        />
        <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' as const }}>
          {LANGUAGES.map(lang => (
            <button key={lang} onClick={() => setFilter(lang)}
              style={{ padding: '6px 14px', borderRadius: '20px', border: '1px solid ' + (filter === lang ? '#1A1A18' : '#DDD8CA'), background: filter === lang ? '#1A1A18' : 'white', color: filter === lang ? 'white' : '#7A7060', fontFamily: F, fontSize: '12px', fontWeight: 300, cursor: 'pointer' }}>
              {lang}
            </button>
          ))}
        </div>
      ...
      <div style={{ maxWidth: '800px', margin: '0 auto', padding: '32px 24px 80px' }}>
        {grouped.length === 0 ? (
          <p style={{ fontFamily: F, fontSize: '14px', fontWeight: 300, color: '#7A7060', textAlign: 'center' as const, marginTop: '48px' }}>No terms found.</p>
