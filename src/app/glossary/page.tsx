...
export default function Glossary() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'transparent' }}>
      {/* Search + filters */}
      <div style={{ background: '#ECE3CC', borderBottom: '1px solid #D9CFAE', padding: '12px 32px', display: 'flex', gap: '12px', flexWrap: 'wrap' as const, alignItems: 'center', position: 'sticky' as const, top: '60px', zIndex: 9 }}>
        ...
        <input
          ...
          style={{ flex: 1, minWidth: '200px', padding: '10px 16px', borderRadius: '10px', border: '1px solid #D9CFAE', background: '#F2EDDF', fontFamily: F, fontSize: 'var(--nl-text-meta)', fontWeight: 400, color: '#2A2318', outline: 'none' }}
        />
        <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' as const }}>
          {LANGUAGES.map(lang => (
            <button key={lang} onClick={() => setFilter(lang)}
              style={{ padding: '6px 14px', borderRadius: '20px', border: '1px solid ' + (filter === lang ? '#1A1A18' : '#D9CFAE'), background: filter === lang ? '#1A1A18' : 'white', color: filter === lang ? 'white' : '#7A7060', fontFamily: F, fontSize: 'var(--nl-text-compact)', fontWeight: 400, cursor: 'pointer' }}>
              {lang}
            </button>
          ))}
        </div>
      ...
      <div style={{ maxWidth: '800px', margin: '0 auto', padding: '32px 24px 80px' }}>
        {grouped.length === 0 ? (
          ...
          <div key={letter} style={{ marginBottom: '32px' }}>
            <div style={{ fontFamily: SERIF, fontSize: '28px', fontWeight: 300, color: '#D9CFAE', marginBottom: '12px', borderBottom: '1px solid #EDE8DF', paddingBottom: '4px' }}>{letter}</div>
            <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '1px' }}>
              {entries.map((entry, i) => (
                <div key={i} style={{ background: '#ECE3CC', borderRadius: i === 0 ? '12px 12px 0 0' : i === entries.length - 1 ? '0 0 12px 12px' : '0', padding: '14px 20px', display: 'flex', gap: '16px', alignItems: 'flex-start' }}>
