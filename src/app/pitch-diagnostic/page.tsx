  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F5F2EC' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 24px', borderBottom: '1px solid #D3D1C7', background: 'white' }}>
        <h1 style={{ fontFamily: SERIF, fontWeight: 300, fontSize: '20px', color: '#1A1A18' }}>Pitch Detection Calibration</h1>
        <div style={{ display: 'flex', gap: '10px' }}>
          {isRunning && <button onClick={abort} style={{ padding: '8px 16px', borderRadius: '10px', border: '1px solid #D3D1C7', background: 'white', fontFamily: F, fontSize: '13px', color: '#888780', cursor: 'pointer' }}>Abort</button>}
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '24px', display: 'grid', gridTemplateColumns: '260px 1fr', gap: '20px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '16px' }}>
          ...
          {/* Instructions */}
          <div style={{ background: isRunning ? '#FAEEDA' : 'white', borderRadius: '16px', border: '1px solid ' + (isRunning ? '#BA7517' : '#D3D1C7'), padding: '20px' }}>
            {phase === 'idle' && (
              <>
                <p style={{ fontFamily: F, fontSize: '11px', color: '#888780', letterSpacing: '0.1em', textTransform: 'uppercase' as const, marginBottom: '8px' }}>Instructions</p>
                <p style={{ fontFamily: F, fontSize: '13px', fontWeight: 300, color: '#1A1A18', lineHeight: 1.7 }}>
            ...
            {(phase === 'dead' || phase === 'listening') && (
              <>
                ...
                <p style={{ fontFamily: SERIF, fontSize: '56px', fontWeight: 300, color: '#1A1A18', lineHeight: 1, marginBottom: '8px' }}>{currentNote}</p>
                <p style={{ fontFamily: F, fontSize: '12px', color: '#888780' }}>Note {noteIdx + 1} of {seq.length}</p>
                {liveDetected && (
            ...
            {phase === 'done' && (
              <>
                ...
                <p style={{ fontFamily: SERIF, fontSize: '28px', fontWeight: 300, color: '#1A1A18' }}>{latestRun?.accuracy}% accurate</p>
                <p style={{ fontFamily: F, fontSize: '12px', color: '#888780', marginTop: '4px' }}>Avg latency: {latestRun?.avgLatencyMs}ms</p>
              </>
            )}
          </div>
          ...
          {isRunning && (
            <div style={{ background: 'white', borderRadius: '8px', border: '1px solid #D3D1C7', padding: '4px' }}>
              <div style={{ height: '6px', borderRadius: '4px', background: '#BA7517', width: `${(noteIdx / seq.length) * 100}%`, transition: 'width 0.3s' }} />
            </div>
          )}
            ...
            <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '6px' }}>
              {Object.keys(SEQUENCES).map(seq => (
                <button key={seq} onClick={() => setSelectedSeq(seq)} disabled={isRunning}
                  style={{ padding: '8px 12px', borderRadius: '8px', border: '1px solid ' + (selectedSeq === seq ? '#1A1A18' : '#D3D1C7'), background: selectedSeq === seq ? '#1A1A18' : 'white', color: selectedSeq === seq ? 'white' : '#888780', fontFamily: F, fontSize: '12px', cursor: isRunning ? 'default' : 'pointer', textAlign: 'left' as const }}>
            ...
            {([
              ...
              <div key={label} style={{ marginBottom: '14px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                  <p style={{ fontFamily: F, fontSize: '11px', fontWeight: 300, color: '#1A1A18' }}>{label}</p>
        ...
        <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '16px' }}>
          ...
          {(isRunning || isDone) && currentResults.length > 0 && (
            <div style={{ background: 'white', borderRadius: '16px', border: '1px solid #D3D1C7', padding: '20px' }}>
              <p style={{ fontFamily: F, fontSize: '10px', color: '#888780', letterSpacing: '0.1em', textTransform: 'uppercase' as const, marginBottom: '12px' }}>Results</p>
              <div style={{ overflowX: 'auto' as const }}>
                <table style={{ width: '100%', borderCollapse: 'collapse' as const }}>
                  <thead>
                    <tr style={{ borderBottom: '1px solid #EDE8DF' }}>
                      {['Target','Detected','Correct','Latency','FP Before','Bleed After','Cents'].map(h => (
                        <th key={h} style={{ padding: '6px 10px', textAlign: 'left' as const, fontFamily: F, fontSize: '10px', color: '#888780', fontWeight: 400, textTransform: 'uppercase' as const, whiteSpace: 'nowrap' as const }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {[...currentResults].reverse().map((r, i) => (
                      <tr key={i} style={{ borderBottom: '1px solid #F5F2EC', background: r.correct ? 'white' : '#FFF5F5' }}>
                        <td style={{ padding: '6px 10px', fontFamily: SERIF, fontSize: '15px' }}>{r.target}</td>
                        <td style={{ padding: '6px 10px', fontFamily: SERIF, fontSize: '15px', color: r.correct ? '#4CAF50' : '#E53935' }}>{r.detected}</td>
                        <td style={{ padding: '6px 10px' }}>
                          <span style={{ padding: '2px 8px', borderRadius: '10px', background: r.correct ? '#E8F5E9' : '#FDECEA', color: r.correct ? '#4CAF50' : '#E53935', fontFamily: F, fontSize: '11px' }}>
                            {r.correct ? '✓' : '✗'}
                          </span>
                        </td>
                        <td style={{ padding: '6px 10px', fontFamily: F, fontSize: '12px', color: r.latencyMs > 400 ? '#E53935' : '#1A1A18' }}>{r.latencyMs}ms</td>
                        <td style={{ padding: '6px 10px', fontFamily: F, fontSize: '12px', color: r.falsePositivesBefore.length > 0 ? '#E53935' : '#888780' }}>
                          {r.falsePositivesBefore.length > 0 ? r.falsePositivesBefore.join(', ') : '—'}
                        </td>
                        <td style={{ padding: '6px 10px', fontFamily: F, fontSize: '12px', color: r.bleedAfter.length > 0 ? '#E53935' : '#888780' }}>
                          {r.bleedAfter.length > 0 ? r.bleedAfter.join(', ') : '—'}
                        </td>
                        <td style={{ padding: '6px 10px', fontFamily: F, fontSize: '12px', color: Math.abs(r.cents) > 20 ? '#E53935' : '#888780' }}>
                ...
                <thead>
                  <tr style={{ borderBottom: '1px solid #EDE8DF' }}>
                    {['Seq','Accuracy','Avg Latency','False Pos','Bleed','Dead Win','Win Size','Stable','Level'].map(h => (
                      <th key={h} style={{ padding: '6px 8px', textAlign: 'left' as const, fontFamily: F, fontSize: '10px', color: '#888780', fontWeight: 400, textTransform: 'uppercase' as const, whiteSpace: 'nowrap' as const }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {runs.map((run, i) => (
                    <tr key={i} style={{ borderBottom: '1px solid #F5F2EC', background: i === 0 ? '#FAFAF8' : 'white' }}>
                      ...
                      <td style={{ padding: '6px 8px', fontFamily: F, fontSize: '12px', color: run.falsePositiveCount > 0 ? '#E53935' : '#888780' }}>{run.falsePositiveCount}</td>
                      <td style={{ padding: '6px 8px', fontFamily: F, fontSize: '12px', color: run.bleedCount > 0 ? '#E53935' : '#888780' }}>{run.bleedCount}</td>
                      <td style={{ padding: '6px 8px', fontFamily: F, fontSize: '12px', color: '#888780' }}>{run.params.deadWindow}ms</td>
                      <td style={{ padding: '6px 8px', fontFamily: F, fontSize: '12px', color: '#888780' }}>{run.params.windowSize}</td>
                      <td style={{ padding: '6px 8px', fontFamily: F, fontSize: '12px', color: '#888780' }}>{run.params.stableThreshold}</td>
                      <td style={{ padding: '6px 8px', fontFamily: F, fontSize: '12px', color: '#888780' }}>{run.params.levelThreshold}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
