  ...
  return (
    <div style={{ minHeight: '100vh', background: '#2C2A27' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 24px', borderBottom: '1px solid #484542', background: '#353330' }}>
        <h1 style={{ fontFamily: SERIF, fontWeight: 300, fontSize: '20px', color: '#F7F4EF' }}>Engine Diagnostic</h1>
        <div style={{ display: 'flex', gap: '10px' }}>
          {running && <button onClick={abort} style={{ padding: '8px 16px', borderRadius: '10px', border: '1px solid #484542', background: '#353330', fontFamily: F, fontSize: '13px', color: '#C4C0B8', cursor: 'pointer' }}>Abort</button>}
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '24px', display: 'grid', gridTemplateColumns: '260px 1fr', gap: '20px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '16px' }}>
          ...
          <div style={{ background: liveStatus === 'correct' ? '#EAF3DE' : liveStatus === 'wrong' ? '#FDECEA' : 'white', borderRadius: '16px', border: '1px solid ' + (liveStatus === 'correct' ? '#7EC86E' : liveStatus === 'wrong' ? '#F09595' : '#484542'), padding: '20px', textAlign: 'center' as const, transition: 'all 0.15s' }}>
            <p style={{ fontFamily: F, fontSize: '10px', color: '#C4C0B8', letterSpacing: '0.1em', textTransform: 'uppercase' as const, marginBottom: '4px' }}>
              {currentPhase === 'dead' ? '⏳ Dead Window' : currentPhase === 'accepting' ? '🎵 Listening' : 'Ready'}
            </p>
            <p style={{ fontFamily: SERIF, fontSize: '52px', fontWeight: 300, color: '#F7F4EF', lineHeight: 1, marginBottom: '8px' }}>{currentNote || '—'}</p>
            {liveDetected && <p style={{ fontFamily: F, fontSize: '14px', color: liveStatus === 'correct' ? '#4CAF50' : liveStatus === 'wrong' ? '#E53935' : '#C4C0B8' }}>Hearing: {liveDetected}</p>}
          </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '6px' }}>
              {Object.keys(SEQUENCES).map(s => (
                <button key={s} onClick={() => setSelectedSeq(s)} disabled={running}
                  style={{ padding: '7px 12px', borderRadius: '8px', border: '1px solid ' + (selectedSeq === s ? '#1A1A18' : '#484542'), background: selectedSeq === s ? '#1A1A18' : 'white', color: selectedSeq === s ? 'white' : '#C4C0B8', fontFamily: F, fontSize: '12px', cursor: running ? 'default' : 'pointer', textAlign: 'left' as const }}>
            ...
            {([
              ...
              <div key={label} style={{ marginBottom: '14px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                  <p style={{ fontFamily: F, fontSize: '11px', color: '#F7F4EF' }}>{label}</p>
        ...
        <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '16px' }}>
          ...
          {currentCards.length > 0 && (
            <div style={{ background: '#353330', borderRadius: '16px', border: '1px solid #484542', padding: '20px' }}>
              <p style={{ fontFamily: F, fontSize: '10px', color: '#C4C0B8', letterSpacing: '0.1em', textTransform: 'uppercase' as const, marginBottom: '12px' }}>Card Results</p>
                  ...
                  <thead>
                    <tr style={{ borderBottom: '1px solid #EDE8DF' }}>
                      {['Target','Prev','Result','Detected','Latency','Total','Dead Win','Cents','First Stable','First ms','FP Dead','Wrongs'].map(h => (
                        <th key={h} style={{ padding: '6px 8px', textAlign: 'left' as const, color: '#C4C0B8', fontWeight: 400, fontSize: '10px', textTransform: 'uppercase' as const, whiteSpace: 'nowrap' as const }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {[...currentCards].reverse().map((c, i) => (
                      <tr key={i} style={{ borderBottom: '1px solid #2C2A27', background: c.outcome === 'correct' ? 'white' : c.outcome === 'wrong' ? '#FFF5F5' : '#FFFBF0' }}>
                        ...
                        <td style={{ padding: '6px 8px', color: '#C4C0B8' }}>{c.prevNote || '—'}</td>
                        ...
                        <td style={{ padding: '6px 8px', color: '#C4C0B8' }}>{c.totalTimeMs}ms</td>
                        <td style={{ padding: '6px 8px', color: '#C4C0B8' }}>{c.deadWindowMs}ms</td>
                        <td style={{ padding: '6px 8px', color: Math.abs(c.centsAtCorrect) > 20 ? '#E53935' : '#C4C0B8' }}>{c.centsAtCorrect ? (c.centsAtCorrect > 0 ? '+' : '')+c.centsAtCorrect+'¢' : '—'}</td>
                        <td style={{ padding: '6px 8px', color: pitchMatch(c.firstStableAfterDead, c.target) ? '#4CAF50' : '#E53935' }}>{c.firstStableAfterDead}</td>
                        <td style={{ padding: '6px 8px', color: '#C4C0B8' }}>{c.firstStableMs >= 0 ? c.firstStableMs+'ms' : '—'}</td>
                        <td style={{ padding: '6px 8px', color: c.fpDuringDead.length > 0 ? '#E53935' : '#C4C0B8', maxWidth: '120px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' as const }} title={c.fpDuringDead.join(', ')}>{c.fpDuringDead.length > 0 ? c.fpDuringDead.slice(-3).join(' ') + (c.fpDuringDead.length > 3 ? '…' : '') : '—'}</td>
                        <td style={{ padding: '6px 8px', color: c.wrongsBeforeCorrect.length > 0 ? '#E53935' : '#C4C0B8' }}>{c.wrongsBeforeCorrect.join(' ') || '—'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <div style={{ background: '#353330', borderRadius: '16px', border: '1px solid #484542', padding: '20px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                  <button onClick={copyAll} style={{ fontFamily: F, fontSize: '11px', color: copied ? '#4CAF50' : '#C4C0B8', background: 'none', border: '1px solid #484542', borderRadius: '6px', padding: '3px 10px', cursor: 'pointer' }}>
                    {copied ? '✓ Copied' : 'Copy All'}
                  </button>
                  <button onClick={() => setRuns([])} style={{ fontFamily: F, fontSize: '11px', color: '#C4C0B8', background: 'none', border: '1px solid #484542', borderRadius: '6px', padding: '3px 10px', cursor: 'pointer' }}>Clear</button>
                </div>
              </div>
                ...
                <thead>
                  <tr style={{ borderBottom: '1px solid #EDE8DF' }}>
                    {['Seq','Accuracy','Avg Latency','Avg Total','Wrong','Timeout','Win','Threshold','Level'].map(h => (
                      <th key={h} style={{ padding: '6px 8px', textAlign: 'left' as const, color: '#C4C0B8', fontWeight: 400, fontSize: '10px', textTransform: 'uppercase' as const }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {runs.map((r, i) => (
                    <tr key={i} style={{ borderBottom: '1px solid #2C2A27', background: i === 0 ? '#FAFAF8' : 'white' }}>
                      ...
                      <td style={{ padding: '6px 8px', color: '#C4C0B8' }}>{r.avgTotalTimeMs}ms</td>
                      <td style={{ padding: '6px 8px', color: r.wrongCount > 0 ? '#E53935' : '#C4C0B8' }}>{r.wrongCount}</td>
                      <td style={{ padding: '6px 8px', color: r.timeoutCount > 0 ? '#E53935' : '#C4C0B8' }}>{r.timeoutCount}</td>
                      <td style={{ padding: '6px 8px', color: '#C4C0B8' }}>{r.params.windowSize}</td>
                      <td style={{ padding: '6px 8px', color: '#C4C0B8' }}>{r.params.stableThreshold}</td>
                      <td style={{ padding: '6px 8px', color: '#C4C0B8' }}>{r.params.levelThreshold}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
