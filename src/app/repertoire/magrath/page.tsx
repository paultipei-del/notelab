...
export default function MagrathBrowser() {
  ...
  if (loading) return (
    <div style={{ display: 'flex', justifyContent: 'center', padding: '60px' }}>
      <p style={{ fontFamily: F, fontSize: 'var(--nl-text-meta)', fontWeight: 400, color: '#7A7060' }}>Loading Magrath data…</p>
    </div>
  )
  ...
  return (
    <div>
      <div style={{ marginBottom: '20px' }}>
        <p style={{ fontFamily: F, fontSize: 'var(--nl-text-compact)', fontWeight: 400, letterSpacing: '0.12em', textTransform: 'uppercase' as const, color: '#7A7060', marginBottom: '4px' }}>Jane Magrath · Alfred Publishing, 1995</p>
        <p style={{ fontFamily: F, fontSize: 'var(--nl-text-meta)', fontWeight: 400, color: '#7A7060' }}>3,141 entries · 552 composers · 4 periods · Levels 1–10</p>
      </div>
              ...
              <thead>
                <tr style={{ borderBottom: '1px solid #EDE8DF' }}>
                  {['Period', 'Composer', 'Title', 'Level', ''].map(h => (
                    <th key={h} style={{ padding: '12px 16px', fontFamily: F, fontSize: 'var(--nl-text-badge)', fontWeight: 400, letterSpacing: '0.1em', textTransform: 'uppercase' as const, color: '#7A7060', textAlign: 'left' as const }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {searchResults.map((r, i) => (
                  <>
                    <tr key={i} onClick={() => setExpandedIdx(expandedIdx === i ? null : i)} style={{ borderBottom: '1px solid #F2EDDF', cursor: 'pointer' }}
                      ...
                      <td style={{ padding: '10px 16px', fontFamily: F, fontSize: 'var(--nl-text-compact)', fontWeight: 400, color: '#7A7060', whiteSpace: 'nowrap' as const }}>{r.period}</td>
                      <td style={{ padding: '10px 16px', fontFamily: F, fontSize: 'var(--nl-text-meta)', fontWeight: 400, color: '#2A2318' }}>{r.entry.composer}</td>
                      <td style={{ padding: '10px 16px', fontFamily: SERIF, fontSize: 'var(--nl-text-body)', fontWeight: 400, color: r.entry.title ? '#1A1A18' : '#7A7060', fontStyle: r.entry.title ? 'normal' : 'italic' as const }}>
                        {r.entry.title || r.entry.description.slice(0, 60) + '…'}
                      </td>
                      <td style={{ padding: '10px 16px', fontFamily: F, fontSize: 'var(--nl-text-compact)', fontWeight: 400, color: '#7A7060' }}>{r.entry.level}</td>
                      <td style={{ padding: '10px 16px', textAlign: 'center' as const }}>
                        <a href={imslpUrl(r.entry.composer, r.entry.title)} target="_blank" rel="noopener noreferrer"
                          ...
                          style={{ color: '#7A7060', fontSize: 'var(--nl-text-ui)', textDecoration: 'none', opacity: 0.6 }}
                    ...
                    {expandedIdx === i && (
                      <tr key={`exp-${i}`}>
                        <td colSpan={4} style={{ padding: '12px 16px 20px 56px', background: '#FAFAF8', borderBottom: '1px solid #EDE8DF' }}>
                          <p style={{ fontFamily: F, fontSize: 'var(--nl-text-compact)', color: '#7A7060', marginBottom: '6px' }}>{r.entry.composer} · {r.entry.dates} · {r.entry.country} · Level {r.entry.level}</p>
                          <p style={{ fontFamily: F, fontSize: 'var(--nl-text-meta)', fontWeight: 400, color: '#2A2318', lineHeight: 1.8, maxWidth: '700px' }}>{r.entry.description}</p>
                        </td>
                      </tr>
                    )}
                  </>
              ...
              <thead>
                <tr style={{ borderBottom: '1px solid #EDE8DF' }}>
                  {([['level_sort', 'Level'], ['composer', 'Composer'], ['title', 'Title']] as const).map(([col, label]) => (
                    <th key={col} onClick={() => toggleSort(col)} style={{ padding: '12px 16px', fontFamily: F, fontSize: 'var(--nl-text-badge)', fontWeight: 400, letterSpacing: '0.1em', textTransform: 'uppercase' as const, color: sortCol === col ? '#B5402A' : '#7A7060', textAlign: 'left' as const, cursor: 'pointer', userSelect: 'none' as const, whiteSpace: 'nowrap' as const }}>
                      {label} {sortCol === col ? (sortDir === 'asc' ? '↑' : '↓') : ''}
                    </th>
                  ))}
                  <th style={{ padding: '12px 16px', fontFamily: F, fontSize: 'var(--nl-text-badge)', fontWeight: 400, letterSpacing: '0.1em', textTransform: 'uppercase' as const, color: '#7A7060', textAlign: 'left' as const }}>Description</th>
                ...
                {filteredEntries.map((entry, i) => (
                  <>
                    <tr key={i} onClick={() => setExpandedIdx(expandedIdx === i ? null : i)}
                      ...
                      <td style={{ padding: '10px 16px', fontFamily: F, fontSize: 'var(--nl-text-compact)', fontWeight: 400, color: '#7A7060', whiteSpace: 'nowrap' as const }}>{entry.level}</td>
                      <td style={{ padding: '10px 16px', fontFamily: F, fontSize: 'var(--nl-text-meta)', fontWeight: 400, color: '#2A2318', whiteSpace: 'nowrap' as const }}>{entry.composer}</td>
                      <td style={{ padding: '10px 16px', fontFamily: SERIF, fontSize: 'var(--nl-text-body)', fontWeight: 400, color: entry.title ? '#1A1A18' : '#7A7060', fontStyle: entry.title ? 'normal' : 'italic' as const }}>
                        {entry.title || <span style={{ fontSize: 'var(--nl-text-meta)' }}>{entry.description.slice(0, 50)}…</span>}
                      </td>
                      <td style={{ padding: '10px 16px', fontFamily: F, fontSize: 'var(--nl-text-compact)', fontWeight: 400, color: '#7A7060', maxWidth: '320px' }}>
                    ...
                    {expandedIdx === i && (
                      <tr key={`exp-${i}`}>
                        <td colSpan={4} style={{ padding: '12px 16px 20px 56px', background: '#FAFAF8', borderBottom: '1px solid #EDE8DF' }}>
                          <p style={{ fontFamily: F, fontSize: 'var(--nl-text-compact)', fontWeight: 400, color: '#7A7060', marginBottom: '8px' }}>{entry.composer} · {entry.dates} · {entry.country} · Level {entry.level}</p>
                          <p style={{ fontFamily: F, fontSize: 'var(--nl-text-meta)', fontWeight: 400, color: '#2A2318', lineHeight: 1.8, maxWidth: '700px' }}>{entry.description}</p>
                        </td>
                      </tr>
                    )}
                  </>
