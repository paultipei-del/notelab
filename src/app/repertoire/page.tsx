  ...
  if (!isPro) {
    return (
      <div style={{ minHeight: '100vh', background: '#F2EDDF' }}>
          ...
          <p style={{ fontFamily: F, fontSize: 'var(--nl-text-ui)', fontWeight: 400, color: '#7A7060', marginBottom: '32px', lineHeight: 1.7 }}>
            Browse graded repertoire lists by level — Preparatory through Advanced. Search by composer or title across all levels.
          </p>
          <button onClick={() => router.push('/')}
            style={{ background: '#1A1A18', color: 'white', border: 'none', borderRadius: '10px', padding: '12px 28px', fontFamily: F, fontSize: 'var(--nl-text-meta)', fontWeight: 400, cursor: 'pointer' }}>
            Upgrade to Pro
          </button>
        </div>
      </div>
    ...
    <div style={{ minHeight: '100vh', background: '#F2EDDF' }}>
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '32px 24px 80px' }}>
        ...
        <div style={{ marginBottom: '32px' }}>
          <button onClick={() => router.push('/tools')} style={{ background: 'none', border: 'none', cursor: 'pointer', fontFamily: F, fontSize: 'var(--nl-text-meta)', fontWeight: 400, color: '#7A7060', padding: 0, marginBottom: '16px', display: 'block' }}>← Back</button>
          <h1 style={{ fontFamily: SERIF, fontWeight: 300, fontSize: '36px', color: '#2A2318', marginBottom: '4px' }}>Repertoire Browser</h1>
          <p style={{ fontFamily: F, fontSize: 'var(--nl-text-meta)', fontWeight: 400, color: '#7A7060' }}>Browse graded repertoire lists by level, or search by composer and title across the full catalog.</p>
        </div>
                  ...
                  <tbody>
                    {searchResults.map((r, i) => (
                      <tr key={i} style={{ borderBottom: i < searchResults.length - 1 ? '1px solid #F2EDDF' : 'none' }}>
                        <td style={{ padding: '10px 16px', fontFamily: F, fontSize: 'var(--nl-text-compact)', fontWeight: 400, color: '#7A7060', whiteSpace: 'nowrap' as const }}>{r.level}</td>
                        <td style={{ padding: '10px 16px', fontFamily: F, fontSize: 'var(--nl-text-compact)', fontWeight: 400, color: '#7A7060', whiteSpace: 'nowrap' as const }}>{r.category}</td>
                        <td style={{ padding: '10px 16px', fontFamily: F, fontSize: 'var(--nl-text-meta)', fontWeight: 400, color: '#2A2318' }}>{r.entry.composer}</td>
                        <td style={{ padding: '10px 16px', fontFamily: SERIF, fontSize: 'var(--nl-text-body)', fontWeight: 400, color: '#2A2318' }}>
                          {r.entry.title}
                          {r.entry.restrictions && <span style={{ marginLeft: '8px', fontFamily: F, fontSize: 'var(--nl-text-compact)', color: '#B5402A', fontStyle: 'italic' }}>{r.entry.restrictions}</span>}
                        </td>
                        <td style={{ padding: '10px 16px', fontFamily: F, fontSize: 'var(--nl-text-compact)', fontWeight: 400, color: '#7A7060' }}>{r.entry.publisher}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            {currentLevel && (
              <>
                ...
                <div style={{ background: '#FDFAF3', borderRadius: '16px', border: '1px solid #DDD8CA', padding: '20px 24px', marginBottom: '20px', display: 'flex', gap: '32px', flexWrap: 'wrap' as const, alignItems: 'flex-start' }}>
                  ...
                  <div>
                    <p style={{ fontFamily: F, fontSize: 'var(--nl-text-badge)', fontWeight: 400, letterSpacing: '0.1em', textTransform: 'uppercase' as const, color: '#7A7060', marginBottom: '4px' }}>Memorized</p>
                  ...
                  {currentLevel.requirements.notes && (
                    <div style={{ flex: 1, minWidth: '200px' }}>
                      <p style={{ fontFamily: F, fontSize: 'var(--nl-text-badge)', fontWeight: 400, letterSpacing: '0.1em', textTransform: 'uppercase' as const, color: '#7A7060', marginBottom: '4px' }}>Notes</p>
                      <p style={{ fontFamily: F, fontSize: 'var(--nl-text-meta)', fontWeight: 400, color: '#7A7060', lineHeight: 1.6 }}>{currentLevel.requirements.notes}</p>
                    </div>
                  )}
                </div>
                        ...
                        <tr style={{ borderBottom: '1px solid #EDE8DF' }}>
                          {(['composer', 'title', 'publisher'] as const).map(col => (
                            <th key={col} onClick={() => toggleSort(col)}
                              style={{ padding: '12px 16px', fontFamily: F, fontSize: 'var(--nl-text-badge)', fontWeight: 400, letterSpacing: '0.1em', textTransform: 'uppercase' as const, color: sortCol === col ? '#B5402A' : '#7A7060', textAlign: 'left' as const, cursor: 'pointer', userSelect: 'none' as const, whiteSpace: 'nowrap' as const }}>
                        ...
                        {sortedEntries.map((entry, i) => (
                          <tr key={i} style={{ borderBottom: i < sortedEntries.length - 1 ? '1px solid #F2EDDF' : 'none', transition: 'background 0.1s' }}
                            ...
                            <td style={{ padding: '10px 16px', fontFamily: SERIF, fontSize: 'var(--nl-text-body)', fontWeight: 400, color: '#2A2318' }}>
                              {entry.title}
                              {entry.restrictions && (
                                <span style={{ marginLeft: '8px', fontFamily: F, fontSize: 'var(--nl-text-compact)', color: '#B5402A', fontStyle: 'italic' as const }}>{entry.restrictions}</span>
                              )}
                            </td>
                            <td style={{ padding: '10px 16px', fontFamily: F, fontSize: 'var(--nl-text-compact)', fontWeight: 400, color: '#7A7060', whiteSpace: 'nowrap' as const }}>{entry.publisher}</td>
